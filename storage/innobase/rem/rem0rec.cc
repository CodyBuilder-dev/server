...
@return total size */
template<bool redundant_temp=false>
	...
	bool			temp)
{
	...
	ulint		fixed_len;
	ulint		null_mask	= 1;
	ulint		n_core_fields = redundant_temp
			? row_log_get_n_core_fields(index)
			: index->n_core_fields;
	ut_ad(n_fields > 0);
...
rec_get_converted_size_temp(
	...
	rec_comp_status_t	status)
{
	return rec_get_converted_size_comp_prefix_low<redundant_temp>(
...
rec_convert_dtuple_to_temp(
	...
	rec_comp_status_t	status)
{
	rec_convert_dtuple_to_rec_comp<redundant_temp>(
